...
Context is the object that Android run time hands over to each Android Application that gets started. It contains all the global information that app needs to know - string resources, image resources, Android components in the app (Activity Service, Broadcast Receiver & Content Provider) etc.

Since context provides access to resources, databases and preferences etc, so  whatever objects inherit the Context object (like activity and services objects), are allowed access to application specific resources and information about the application environment. *Both the Activity and Application classes extend the Context class, so they inherit the context*.

Two main contexts in Android:
**Application Context**
It is an instance which is **singleton** and can be accessed in an activity via **getApplicationContext()**. This context is tied to the lifecycle of an application. The application context can be used where you need a context whose lifecycle is separate from the current context or when you are passing a context beyond the scope of an activity.
...
Example: For any UI operations like showing toast, dialogs, and etc, use the Activity Context.

* NOTE: **MyApplication** any class extending to Application will have only Application context present.But Activities have  both Activity context and Application context present. Here, the nearest Context is Activity Context.
* Usage: 
1. Suppose we have our class MyApplication(which extends the Application class). And, another class ```MyDB``` which is Singleton. And ```MyDB``` needs context. Which context will we be passing: **Application Context**, since MyDB is singleton, only one instance of the class can be created, so its better to provide it application context, so that it has a context which is not dependent on the activity lifecycle. otherwise, even when the activity is finished, due to this MyDB class having context through Activity, the activity will not be garbage collected, causing memory leak.

When not to use Application Context: 
...
**getBaseContext()**
Let's first understand ```ContextWrapper```. Android provides a ```ContextWrapper``` class that is created around an existing Context using:
```
...
**The Context referred to from inside that ContextWrapper is accessed via ```getBaseContext()```.**. The method getBaseContext() can be used to access the “base” Context that the ContextWrapper wraps around.

* **What is `AndroidManifest.xml`?**
 Android Manifest is a file which some kind of meta information of our application which can be read by the system, android build tools and google play.
 Among other things Manifest file is required to declare the following:
 1. Package name of our application:(Generally same as code namespace but not mandatory.) Android Build tool uses this to determine location of entities, while building the project.While building package name is replaced with unique applicationId, which can uniquely identify them on system and google play.
 2. Name of Components that the application is using including all Activities, Services, Broadcast receivers and content providers.
 3. Permissions that the application will needs in order to access the restricted parts of the system like accessing device location, reading sms, camera etc.
 4. The hardware features(```<uses-feature/>```) and software features (```<uses-sdk/>```) that the device is using. Google play can read these and determine which devices can install this app and which can not.
...
- We cannot change application class but we could give additional instruction to it by extending it.
...
- This class is primarily used for:
  1. Performing tasks that need to run before the creation of your first activity like initialization of global values that needs to be shared across all components (crash reporting, persistence)
  2. Static methods for easy access to static immutable data such as : shared network client object, shared pref
...
```onCreate()```: The onCreate() is called only once during the activity lifecycle, either when the application starts, or when the activity has been destroyed and then recreated, for example during a configuration change. This indicates that the activity is being launched either because another component said so, or system is recreating it. We should do one time global declarations of class here, like binding views etc.

```onStart()```: The onStart() method can be called multiple times during the lifecycle of the activity. It is called whenever the activity becomes visible to the user. This method indicates that the activity is becoming visible to user again. We must perform tasks here, which needs to updated after the activity became completely invisible, and is gaining control again.
...
- By calling ```setContentView(R.layout.layout_main)```,we inflate the of layout_main.xml file (```R``` denotes the resources folder, and ```layout``` denotes the layout folder in resources), to our current activity.
- Later we can bind the views of xml layout to our java class, to control how they interact with user.

* **What is onSavedInstanceState() and onRestoreInstanceState() in activity?**
- Any configuration change will cause Android to restart your Activity. A configuration change might be
  1. the device rotating (because now we have a different screen layout to draw upon),
  2. a language switch (because we need to re-write all those strings, which may need more room now OR it could be the scary RTL switch!)
