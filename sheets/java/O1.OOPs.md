...
* static blocks (also called static clause) can be used for static initializations of a class. This code inside static block is executed only once: the first time you make an object of that class or the first time you access a static member of that class. For example: 
```java
public class Test {
    static int a = 10;
    static int b;
...
* Static blocks are also executed only once. IN the following code, it can seen that the static block is executed only one, as soon as the class is loaded. 
```java
class Test {
    static int i;
    int j;
