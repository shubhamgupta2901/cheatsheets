...
* The above operations are performed by Mark and Sweep Algorithm in two phases:
  ...
  2) Sweep phase
* **Mark phase** 
  * When an object is created, its marked bit is set to false. In the Mark phase, we set the marked bit for all the reachable objects (or the objects which a user can refer to) to true.
  ...
  * Root is a variable that refer to an object and is directly accessible by local variable. We will assume that we have one root only.If we have more than one root, then we simply have to call ```mark()``` for all the root variables.
